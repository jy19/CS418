...
class Node:
    def __init__(self, start, end):  # start, end indices so can just pull from text
        self.start = start
        self.end = end
        self.edges = {}  # key: single char, value: node
        self.link = 0
    ...
    def add_char(self, c):
        ...
        while self.r > 0:
            ...
            if self.get_active_edge() not in self.nodes[self.active_node].get_edges():
                # the edge doesn't exist, create a new leaf node
                leaf = self.new_node(self.pos, oo)
                # add the leaf node and use the active_edge as the key for the edge
                ...
                # if successfully walk_down nex
                if self.walk_down(nex):
                    # move on in the algorithm, don't add suffixes
                    continue
                ...
                # creates  new leaf, node( current_position, #)
                leaf = self.new_node(self.pos, oo)
            ...
            # else
                ...
                self.active_node = self.nodes[self.active_node].get_link() \
                    if self.nodes[self.active_node].get_link() > 0 else self.root
